...
export default (app: Router) => {
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      logger.debug('Calling Sign-In Via Google endpoint with body: %o', req.body)
      try {
        const { email, password } = req.body;
        const authServiceInstance = Container.get(AuthService);
        const result = await authServiceInstance.SignInViaGoogle(email);
  ...
  route.patch(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      const logger = Container.get('logger') as winston.Logger;
      logger.debug('Calling Anonymize endpoint with body: %o', req.body )

      try {
        const authServiceInstance = Container.get(AuthService);
        const userOrError = await authServiceInstance.anonymize(req.body as IUserDTO);
  ...
  route.get('',
    async (req: Request, res: Response, next: NextFunction) => {
      const logger = Container.get('logger') as winston.Logger;
      logger.debug('Calling getUsers endpoint with body: %o', req.body )

      try {
        const authServiceInstance = Container.get(AuthService);
        const usersDto = await authServiceInstance.getUsers() as Array<IUserDTO>;
