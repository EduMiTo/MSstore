...
export default class UserService implements IUserService{
  ...
  public async SignUp(userDTO: IUserDTO): Promise<Result<{ userDTO: IUserDTO, token: string }>> {
    try {
      ...
      const userDocument2 = await this.userRepo.findByPhone(userDTO.phone);
      const found2 = !!userDocument2;

      if (found2) {
        return Result.fail<{userDTO: IUserDTO, token: string}>("User already exists with phone number: " + userDTO.phone);
      }
      ...
      if( phoneOrError.isFailure ){
       return Result.fail<{userDTO: IUserDTO; token: string}>(phoneOrError.error);
      }else{
  ...
  public async anonymize(userDTO: IUserDTO): Promise<Result<{ userDTO: IUserDTO}>> {
    try {
      const userDocument = await this.userRepo.findById( userDTO.domainId );
      
      userDocument.firstName = 'Dummy';
      userDocument.lastName = 'Dummy';

      userDocument.phone = UserPhone.anonymize();
      userDocument.email = UserEmail.anonymize();

      await this.userRepo.save(userDocument);
      const userDTOResult = UserMap.toDTO( userDocument ) as IUserDTO;
  ...
  public async getUsers(): Promise<Array<IUserDTO>> {
    let usersResultDto: IUserDTO[] = [];
    try {

      usersResultDto = await this.userRepo.findAll();
  ...
  public async SignInViaGoogle(email: string): Promise<Result<{ userDTO: IUserDTO, token: string }>> {
  ...
  private async getRole (roleId: string): Promise<Result<Role>> {
    ...
    const role = await this.roleRepo.findByDomainName( roleId );
    const found = !!role;
